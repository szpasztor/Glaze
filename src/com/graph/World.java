...
import com.graph.com.graph.types.Camera;
import com.graph.com.graph.types.Edge;
import com.graph.com.graph.types.Graph;
...
public class World {
    ...
    /** Adds a new vertex and creates graph.neighbors from it to the selected graph.vertices */
    ...
    /** Deletes the selected graph.vertices */
    ...
    /**
     * Creates or removes all graph.neighbors between selected graph.vertices.
    ...
    /** Creates/removes an edge between the two parameter graph.vertices.
     * @param a The index of the first vertex (in graph.vertices array).
     * @param b The index of the second vertex (in graph.vertices array).
     * @param create True -> create edge; false -> removeVertex edge
     * @return
     */
    ...
    /** Turns selected non-fixed graph.vertices to fixed ones, fixed graph.vertices to non-fixed ones. */
    public void fix() {
        for (Vertex vertex : graph.vertices) {
            if (vertex.selected) {
    ...
    public void logGraphInf() {
        ...
        /*Gdx.app.log(TAG, "graph.vertices: ");
        for (int i=0;i<graph.vertices.size;i++) {
